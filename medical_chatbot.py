...
#import together
#os.environ["TOGETHER_API_KEY"] = "76eaf508f2b7c43a47b33988c49a6aacae6d245ca9e56796a6b21720a1cd0346"
...
def is_medical_query(query, conversation_history=""):
    ...
    """
    medical_keywords = [
    "disease", "diagnosis", "symptom", "treatment", "medicine", "health", "infection", "diabetes", "cure", "illness",
    "disorder", "fever", "pain", "surgery", "doctor", "hospital", "prescription", "virus", "bacteria", "cancer",
    "flu", "therapy", "vaccine", "injury", "allergy", "nausea", "vomiting", "rash", "antibiotic", "migraine",
    "asthma", "covid", "cholesterol", "blood pressure", "heartbeat", "pulse", "insulin", "cardiac", "urine", "stool",
    "scan", "x-ray", "mri", "ct", "tumor", "fracture", "burn", "wound", "eczema", "arthritis", "anemia", "depression",
    "anxiety", "fatigue", "acne", "hepatitis", "pneumonia", "stroke", "cough", "cold", "sneeze", "itching",
    "swelling", "dizziness", "bleeding", "oxygen", "inhaler", "tablet", "capsule", "ointment", "bandage", "pulse ox",
    "glucose", "neurology", "cardiology", "oncology", "immunology", "psychiatry", "gynecology", "dermatology",
    "pathology", "biopsy", "ultrasound", "monitor", "ventilator", "IV", "dose", "side effect", "pharmacy", "clinic",
    "checkup", "emergency", "triage", "ambulance", "first aid", "discharge", "specialist", "infection control",
    "prognosis", "consultation", "symptomatic", "prescribe"
]
...
def get_response(user_question):
    ...
    if is_medical_query(user_question, conversation_history):
        logging.info("🕒 Medical query detected")

        t0 = time.time()
        context = extract_relevant_medical_text(user_question)
        logging.info(f"⏱️ FAISS search took {time.time() - t0:.2f} sec")

        t1 = time.time()
        biobert_fact = get_biobert_answer(context, user_question)
        logging.info(f"⏱️ BioBERT extraction took {time.time() - t1:.2f} sec")
...
User Query: {user_question}
        ...
        logging.info("🕒 General query detected")
...
User Query: {user_question}
    ...
    logging.info(f"⏱️ Ollama response took {time.time() - t2:.2f} sec")

    memory.save_context({"input": user_question}, {"output": response})
    logging.info(f"✅ Total get_response() took {time.time() - start_total:.2f} sec")
